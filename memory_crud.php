...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $endpoint === '/upload') {
    ...
    if ($skill_id === null) {
        ...
        // Insert memory response into notification table
        $insertNotificationQuery = "INSERT INTO notification (user_id, description) VALUES (?, ?)";
        $stmt = $conn->prepare($insertNotificationQuery);

        // For the current user
        $memory_notification = "Memory added successfully. You earned 2 points!";
        $stmt->bind_param("is", $user_id, $memory_notification);
        $stmt->execute();
    } else {
        $memoryResponse = [
            "status" => "success",
        ...
        // For the current user
        $memory_notification = "Memory added successfully";
        $stmt->bind_param("is", $user_id, $memory_notification);
        $stmt->execute();
    }

    // Integrate Session Logic Here
    if ($skill_id !== null) {
        ...
        if ($learner_streak == 0 && $teacher_streak == 0 && empty($learner_timestamps) && empty($teacher_timestamps)) {
            ...
            // Insert session status into notification table
            $insertNotificationQuery = "INSERT INTO notification (user_id, description) VALUES (?, ?)";
            $stmt = $conn->prepare($insertNotificationQuery);

            // For the learner
            $learner_notification = "Session ongoing (no memories uploaded yet). Remaining streak days: {$remaining_streak_days}. Next streak start time: {$next_streak_start_formatted}";
            $stmt->bind_param("is", $learner_id, $learner_notification);
            $stmt->execute();

            // For the teacher
            $teacher_notification = "Session ongoing (no memories uploaded yet). Remaining streak days: {$remaining_streak_days}. Next streak start time: {$next_streak_start_formatted}";
            ...
            // For the learner
            $learner_notification = "Session ongoing. Learner streak: {$learner_streak}, Teacher streak: {$teacher_streak}, Remaining streak days: {$remaining_streak_days}, Next streak start time: {$next_streak_start_formatted}";
            $stmt->bind_param("is", $learner_id, $learner_notification);
            $stmt->execute();

            // For the teacher
            $teacher_notification = "Session ongoing. Learner streak: {$learner_streak}, Teacher streak: {$teacher_streak}, Remaining streak days: {$remaining_streak_days}, Next streak start time: {$next_streak_start_formatted}";
            $stmt->bind_param("is", $teacher_id, $teacher_notification);
            $stmt->execute();
        }
    }

    // Combine Memory and Session Responses
    $finalResponse = array_merge($memoryResponse, $sessionResponse ?? []);
